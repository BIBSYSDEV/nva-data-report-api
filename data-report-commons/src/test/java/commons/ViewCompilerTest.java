...
class ViewCompilerTest {
  ...
  private static final Path NON_APPLICABLE_NVI_CANDIDATE_NT =
      Path.of("nonApplicableNviCandidate.nt");
  ...
  void shouldReduceTriplesToPublicationViewRequiredToProduceApiDataWithInputModel() {
    ...
    Assertions.assertTrue(
        expected(ACADEMIC_ARTICLE_NT, uri).isIsomorphicWith(actualModelWithAppliedView));
  }
  ...
  private static Model expected(Path tripleFile, URI uri) {
    var triples =
        stringToStream(stringFromResources(tripleFile).replace("__REPLACE_ID__", uri.toString()));

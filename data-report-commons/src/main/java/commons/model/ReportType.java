...
public enum ReportType {
  ...
  public static final String BAD_REQUEST_MESSAGE_TEMPLATE =
      "Illegal argument. Acceptable report types: %s";
  ...
  public static List<ReportType> getAllTypesExcludingNviReport() {
    return Arrays.stream(values()).filter(ReportType::isNotNviReportType).toList();
  }
  ...
  private static String getValidReportTypes() {
    return Arrays.stream(values()).map(ReportType::getType).collect(Collectors.joining(DELIMITER));
  }
}

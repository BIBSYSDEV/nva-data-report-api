...
public final class CsvFormatter implements ResponseFormatter {
  ...
  private static final CSVFormat CSV_FORMAT =
      CSVFormat.Builder.create().setEscape('\\').setQuoteMode(QuoteMode.MINIMAL).build();
  ...
  private static void printRow(
      CSVPrinter csvPrinter, List<String> headers, QuerySolution querySolution) throws IOException {
  ...
  private static void printHeaders(CSVPrinter csvPrinter, List<String> resultVars)
      throws IOException {
    csvPrinter.printRecord(resultVars);
  }

  private static String formatDouble(Literal literal) {
    return new DecimalFormat(DECIMAL_PATTERN, DecimalFormatSymbols.getInstance())
        .format(literal.getDouble());
  }

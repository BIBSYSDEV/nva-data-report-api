...
public record NviIndexDocument(
  ...
  private static List<Approval> generateApprovals(SampleNviCandidate nviCandidate) {
    return nviCandidate.approvals().stream()
        .map(
            testApproval ->
                Approval.from(testApproval, nviCandidate.globalApprovalStatus().getValue()))
        .toList();
  }
    ...
    private record Points(
      ...
      static Points from(SampleInstitutionPoints points) {
        return new Points(
            ...
            points.creatorAffiliationPoints().stream()
                .map(CreatorAffiliationPoints::from)
                .toList());
      }

      private record CreatorAffiliationPoints(
          String type, URI nviCreator, URI affiliationId, double points) {
  ...
  private record PublicationDetails(String id, List<NviContributor> contributors) {

    static PublicationDetails from(SamplePublicationDetails samplePublicationDetails) {
      return new PublicationDetails(
          samplePublicationDetails.id(),
          samplePublicationDetails.contributors().stream().map(NviContributor::from).toList());
    }

    private record NviContributor(String type, String id, List<NviOrganization> affiliations) {

      static final String TYPE = "NviContributor";

      static NviContributor from(SampleNviContributor sampleNviContributor) {
        return new NviContributor(
            TYPE,
            sampleNviContributor.id(),
            sampleNviContributor.affiliations().stream().map(NviOrganization::from).toList());
      }

      private record NviOrganization(String type, String id, List<String> partOf) {

        static final String TYPE = "NviOrganization";

        static NviOrganization from(SampleNviOrganization sampleNviOrganization) {
          return new NviOrganization(
              TYPE, sampleNviOrganization.id(), sampleNviOrganization.partOf());
        }
      }
    }
  }

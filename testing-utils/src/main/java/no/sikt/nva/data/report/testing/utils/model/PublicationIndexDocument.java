...
public record PublicationIndexDocument(
    String type,
    ...
    List<TestOrganization> topLevelOrganizations)
  ...
  private static List<TestOrganization> generateTopLevelOrganizations(
      SamplePublication publication) {
    return publication.getContributorAffiliations().stream()
        .map(PublicationIndexDocument::generateTopLevelOrganization)
        .map(
            json ->
                attempt(() -> dtoObjectMapper.readValue(json, TestOrganization.class))
                    .orElseThrow())
        .toList();
  }

  private static String generateTopLevelOrganization(SampleOrganization affiliation) {
    // Struggled to recreate the nested structure for top level organization based on affiliation,
    // therefore
    // hardcoded
  ...
  private record TestOrganization(
      String id,
      String type,
  ...
  private record EntityDescription(
      String type,
    ...
    private record Contributor(
      ...
      private record Identity(String type, String name, String id) {

        static final String TYPE = "Identity";

        static Identity from(SampleIdentity identity) {
          return new Identity(TYPE, identity.name(), identity.uri());
        }
      }
      ...
      private record Affiliation(
          String id, String type, Map<String, String> labels, Affiliation partOf) {

        static final String TYPE = "Organization";

        static Affiliation from(SampleOrganization sampleOrganization) {
    ...
    private record PublicationDate(String type, String year, String month, String day) {
      ...
      static PublicationDate from(
        ...
        return new PublicationDate(TYPE, date.year(), date.month(), date.day());
      }
    }
    ...
    private record Reference(
      ...
      static Reference from(SamplePublication publication) {
        return new Reference(
            TYPE, PublicationContext.from(publication), PublicationInstance.from(publication));
      }
  ...
  private record Funding(String type, String id, String identifier, FundingSource source) {
    ...
    private record FundingSource(String identifier, String type, Map<String, String> labels) {

      static final String TYPE = "FundingSource";

      static FundingSource from(SampleFunding sampleFunding) {
        return new FundingSource(
            sampleFunding.getFundingSource(), TYPE, Map.of(EN, sampleFunding.getName()));
      }
    }
  }

  private record AdditionalIdentifier(String sourceName, String value, String type) {

    static AdditionalIdentifier from(SampleAdditionalIdentifier additionalIdentifier) {
      return new AdditionalIdentifier(
          additionalIdentifier.getSourceName(),
          additionalIdentifier.getValue(),

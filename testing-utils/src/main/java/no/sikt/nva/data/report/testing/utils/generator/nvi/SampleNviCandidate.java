...
public record SampleNviCandidate(
    String candidateUri,
    String identifier,
  ...
  private ApprovalGenerator getApprovalGenerator(SampleApproval sampleApproval) {
    var institutionPointsGenerator =
        new InstitutionPointsGenerator()
  ...
  private void addApprovals(CandidateGenerator nviCandidate) {
    approvals().stream().map(this::getApprovalGenerator).forEach(nviCandidate::withApproval);
  }
  ...
  private void addCreatorAffiliationPoints(
      SampleApproval approval, InstitutionPointsGenerator institutionPointsGenerator) {
    approval
        .points()
        .creatorAffiliationPoints()
        .forEach(
            creatorAffiliationPoints ->
                institutionPointsGenerator.withCreatorAffiliationPoints(
  ...
  private void generateExpectedNviResponse(
      StringBuilder stringBuilder, SampleNviContributor contributor) {
    contributor
        .affiliations()
        .forEach(
            affiliation -> generateExpectedNviResponse(stringBuilder, contributor, affiliation));
  }
  ...
  private void generateExpectedNviResponse(
    ...
    stringBuilder
        .append(publicationDetails().id())
        .append(DELIMITER)
        .append(extractLastPathElement(contributor.id()))
        .append(DELIMITER)
        .append(affiliation.id())
        .append(DELIMITER)
        .append(approval.institutionId())
        .append(DELIMITER)
        .append(approval.points().institutionPoints())
        .append(DELIMITER)
        ...
        .append(DELIMITER)
        .append(globalApprovalStatus.getValue())
        .append(DELIMITER)
        .append(reported ? reportingPeriod : "NotReported")
        .append(DELIMITER)
        .append(totalPoints)
        .append(DELIMITER)
        .append(publicationTypeChannelLevelPoints)
        .append(DELIMITER)
        .append(creatorShareCount)
        .append(DELIMITER)
        .append(internationalCollaborationFactor)
        .append(DELIMITER)
        .append(isApplicable())
        .append(DELIMITER)
        .append(modifiedDate)
